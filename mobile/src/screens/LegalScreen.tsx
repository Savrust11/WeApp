/**
 * LegalScreen — faithful 1:1 port of the canonical web page
 * `WeYu/client/src/pages/Legal.tsx` (プライバシーポリシー・利用規約).
 *
 * Reproduces ALL of the web content verbatim:
 *  • Title that switches with the active tab
 *    (プライバシーポリシー / サービス利用規約)
 *  • Two pill tabs: プライバシーポリシー / 利用規約
 *  • White rounded content card containing the full policy / terms text
 *    – PrivacyPolicy() : 第1条〜第13条 + お問い合わせ先
 *    – TermsOfService(): 第1条〜第14条 + お問い合わせ先
 *
 * Legal text is reproduced word-for-word (no summary, no paraphrase).
 * Styling sourced ONLY from ../theme/tokens + ../theme/ui; icons from
 * lucide-react-native (same icon WeYu Legal.tsx uses). The screen is
 * registered by the navigator with headerShown: true (back header), so the
 * in-content header here mirrors WeYu's own in-page title row.
 */
...
// ─── Shared text primitives (web: Legal.tsx SectionTitle / Paragraph / List) ───
...
// ─── Privacy Policy (web: PrivacyPolicy) ────────────────────────────────────────

function PrivacyPolicy() {
  return (
    <View>
      <Text style={styles.docTitle}>We育（ウィーイク）</Text>
      <Text style={styles.docSubtitle}>プライバシーポリシー</Text>
      <Text style={styles.docMeta}>制定日：2026年3月4日{'\n'}産前産後ケアホテル ぶどうの木</Text>

      <Paragraph>
        本プライバシーポリシー（以下「本ポリシー」）は、産前産後ケアホテル ぶどうの木（以下「当社」）が提供する育児支援アプリケーション「We育（ウィーイク）」（以下「本アプリ」）における、ユーザーの個人情報およびプライバシー情報の取り扱いについて定めるものです。
      </Paragraph>
      <Paragraph>
      ...
      <SubLabel>1-1. ユーザーが直接提供する情報</SubLabel>
      <List
        items={[
      ...
      />
      <SubLabel>1-2. 自動的に取得する情報</SubLabel>
      <List
        items={[
      ...
      <View style={styles.contactBlock}>
        <Text style={styles.contactLine}>事業者名：産前産後ケアホテル ぶどうの木</Text>
        <Text style={styles.contactLine}>サービス名：We育（ウィーイク）</Text>
        <Text style={styles.contactLine}>お問い合わせ方法：@we-iku</Text>
      </View>

      <Text style={styles.docFooter}>2026年3月4日 制定</Text>
    </View>
  );
}

// ─── Terms of Service (web: TermsOfService) ─────────────────────────────────────

function TermsOfService() {
  return (
    <View>
      <Text style={styles.docTitle}>We育（ウィーイク）</Text>
      <Text style={styles.docSubtitle}>サービス利用規約</Text>
      <Text style={styles.docMeta}>制定日：2026年3月4日{'\n'}産前産後ケアホテル ぶどうの木</Text>

      <Paragraph>
        本利用規約（以下「本規約」）は、産前産後ケアホテル ぶどうの木（以下「当社」）が提供する育児支援アプリケーション「We育（ウィーイク）」（以下「本アプリ」）の利用に関する条件を定めるものです。ユーザーの皆さまには、本規約に同意のうえ、本アプリをご利用いただきます。
      </Paragraph>
...
export default function LegalScreen() {
  ...
  return (
    ...
    >
      {/* Title row — web Legal: text-xl font-black text-purple-800 (switches by tab) */}
      ...
      {/* Pill tabs — web Legal: two rounded-full buttons */}
      <View style={styles.tabs}>
        <TouchableOpacity
          style={[styles.tabPill, tab === 'privacy' ? styles.tabPillActive : styles.tabPillIdle]}
          onPress={() => setTab('privacy')}
          activeOpacity={0.8}
        >
          <Text style={tab === 'privacy' ? styles.tabPillTextActive : styles.tabPillTextIdle}>
            プライバシーポリシー
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.tabPill, tab === 'terms' ? styles.tabPillActive : styles.tabPillIdle]}
          onPress={() => setTab('terms')}
          activeOpacity={0.8}
        >
          <Text style={tab === 'terms' ? styles.tabPillTextActive : styles.tabPillTextIdle}>
            利用規約
          </Text>
        </TouchableOpacity>
      </View>

      {/* Content card — web Legal: bg-white rounded-[24px] p-6 shadow-sm */}
...
const styles = StyleSheet.create({
  ...
  // Pill tabs (web: rounded-full px-4 py-2 text-sm font-bold)
  ...
  tabPillActive: { backgroundColor: palette.primary },
  tabPillIdle: {
  ...
  // Content card (web: bg-white rounded-[24px] p-6 shadow-sm border)
  docCard: {
    backgroundColor: palette.card,
  ...
  // Document heading (web: h2 text-lg font-black text-purple-900 etc.)
  docTitle: {
    fontSize: 18,
    fontFamily: fonts.sans,
    fontWeight: '700',
  ...
  // SectionTitle (web: h2 text-base font-black text-purple-800 mt-6 mb-2)
  sectionTitle: {
    fontSize: 16,
    fontFamily: fonts.sans,
    fontWeight: '700',
  ...
  // Paragraph (web: text-sm text-gray-700 leading-relaxed mb-3)
  paragraph: {
    fontSize: 14,
  ...
  // Sub label (web: text-sm font-bold text-gray-800 mb-1)
  subLabel: {
    fontSize: 14,
    fontFamily: fonts.bodyBold,
    fontWeight: '700',
  ...
  // List (web: list-disc list-inside text-sm text-gray-700 space-y-1)
  list: { marginBottom: 12, gap: 4, paddingLeft: 8 },
  ...
  // Contact block (web: text-sm text-gray-700 pl-2)
  contactBlock: { marginBottom: 12, paddingLeft: 8 },
  contactLine: {
    fontSize: 14,
    color: palette.foreground,
    lineHeight: 22,
  ...
  // Footer (web: text-xs text-gray-500 mt-6 text-right)
  docFooter: {
    fontSize: 12,
    color: palette.mutedForeground,
    marginTop: 24,
